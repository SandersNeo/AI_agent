...
Функция НайтиПоследнийДиалогПользователяПоТипу(Пользователь, ТипДиалога) Экспорт
	...
	Возврат ИИА_Сервер.НайтиПоследнийДиалогПользователяПоТипу(Пользователь, ТипДиалога);
...
Функция ДобавитьВложениеИзФайла(СсылкаДиалога, ПутьКФайлу) Экспорт
	Возврат ИИА_Вложения.ДобавитьВложениеИзФайла(СсылкаДиалога, ПутьКФайлу);
КонецФункции

Функция ДобавитьВложениеИзДвоичныхДанных(СсылкаДиалога, ИмяФайла, ДвоичныеДанныеФайла) Экспорт
	Возврат ИИА_Вложения.ДобавитьВложениеИзДвоичныхДанных(СсылкаДиалога, ИмяФайла, ДвоичныеДанныеФайла);
КонецФункции

Функция ДобавитьВложениеИзВременногоХранилища(СсылкаДиалога, ИмяФайла, АдресВременногоХранилища) Экспорт
	Возврат ИИА_Вложения.ДобавитьВложениеИзВременногоХранилища(СсылкаДиалога, ИмяФайла, АдресВременногоХранилища);
КонецФункции

Функция ПолучитьВложенияДиалога(СсылкаДиалога) Экспорт
	Возврат ИИА_Вложения.ПолучитьВложенияДиалога(СсылкаДиалога);
КонецФункции

Функция ПолучитьПолныйТекстВложения(СсылкаДиалога, ИдентификаторИлиИмя = "") Экспорт
	Возврат ИИА_Вложения.ПолучитьПолныйТекстВложения(СсылкаДиалога, ИдентификаторИлиИмя);
КонецФункции

Функция НайтиВоВложениях(СсылкаДиалога, ИскомыйТекст, Лимит = 20, ИдентификаторИлиИмя = "") Экспорт
	Возврат ИИА_Вложения.НайтиВоВложениях(СсылкаДиалога, ИскомыйТекст, Лимит, ИдентификаторИлиИмя);
КонецФункции
...
Функция ПроверитьРезультатВыполненияЗадачи(СсылкаДиалога) Экспорт
	...
	Возврат ИИА_ВалидаторРезультата.ПроверитьРезультатВыполненияЗадачи(СсылкаДиалога);
...
Функция ПолучитьНевыполненныеЗадачиПользователя(СсылкаДиалога) Экспорт
	...
	Возврат ИИА_TypedPlan.ПолучитьНевыполненныеЗадачиПользователя(СсылкаДиалога);
...
Функция СгенерироватьSummary(СсылкаДиалога) Экспорт
	...
	Возврат ИИА_Summary.СгенерироватьSummary(СсылкаДиалога);
...
Функция ПолучитьПоследниеДанныеЗапроса(СсылкаДиалога) Экспорт
	...
	Возврат ИИА_Сервер.ПолучитьПоследниеДанныеЗапроса(СсылкаДиалога);
	
КонецФункции

Функция ПолучитьСостояниеЗапроса1С(СсылкаДиалога) Экспорт
	
	Возврат ИИА_Сервер.ПолучитьСостояниеЗапроса1С(СсылкаДиалога);
	
КонецФункции

Функция ВыполнитьЗапрос1СИзФормы(СсылкаДиалога, ТекстЗапроса, ПараметрыJSON = "") Экспорт
	
	Возврат ИИА_Сервер.ВыполнитьЗапрос1СИзФормы(СсылкаДиалога, ТекстЗапроса, ПараметрыJSON);
...
Функция ЭтоДиалогЗапрос1С(СсылкаДиалога) Экспорт
	...
	Возврат ИИА_Сервер.ЭтоДиалогЗапрос1С(СсылкаДиалога);
...
Функция ИнициализироватьПлан(СсылкаДиалога) Экспорт
	...
	Возврат ИИА_TypedPlan.ИнициализироватьПлан(СсылкаДиалога);
...
Функция ОбновитьПланПоРезультатам(СсылкаДиалога) Экспорт
	...
	Возврат ИИА_TypedPlan.ОбновитьПланПоРезультатам(СсылкаДиалога);
...
Функция ПолучитьСледующийПунктПлана(СсылкаДиалога) Экспорт
	...
	Возврат ИИА_TypedPlan.ПолучитьСледующийПунктПлана(СсылкаДиалога);
...
Функция ПланЗавершен(СсылкаДиалога) Экспорт
	...
	Возврат ИИА_TypedPlan.ПланЗавершен(СсылкаДиалога);
...
Функция ПолучитьПланДиалога(СсылкаДиалога) Экспорт
	...
	Возврат ИИА_TypedPlan.ПолучитьПланДиалога(СсылкаДиалога);
...
Функция ПолучитьСостояниеПодтверждения(СсылкаДиалога) Экспорт
	Возврат ИИА_Сервер.ПолучитьСостояниеПодтверждения(СсылкаДиалога);
КонецФункции

Функция ОркестраторПодтвердитьОжидающееДействие(СсылкаДиалога) Экспорт
	Возврат ИИА_ApprovalPolicy.ПодтвердитьОжидающееДействие(СсылкаДиалога);
КонецФункции

Функция ОркестраторОтклонитьОжидающееДействие(СсылкаДиалога) Экспорт
	Возврат ИИА_ApprovalPolicy.ОтклонитьОжидающееДействие(СсылкаДиалога);
КонецФункции

Функция ОркестраторВыполнятьБезПодтверждения(СсылкаДиалога) Экспорт
	Возврат ИИА_ApprovalPolicy.ВключитьРежимБезПодтвержденияИПродолжить(СсылкаДиалога);
КонецФункции

Функция ЭтоРежимБезПодтверждения(СсылкаДиалога) Экспорт
	Возврат ИИА_Сервер.ЭтоРежимБезПодтверждения(СсылкаДиалога);
КонецФункции

Функция ПолучитьDecisionTimeline(СсылкаДиалога, Лимит = 20) Экспорт
	Возврат ИИА_Сервер.ПолучитьDecisionTimeline(СсылкаДиалога, Лимит);
КонецФункции

Функция ПолучитьRuntimeTrace(СсылкаДиалога, Лимит = 100) Экспорт
	Возврат ИИА_Сервер.ПолучитьRuntimeTrace(СсылкаДиалога, Лимит);
КонецФункции

Функция ПолучитьИсториюЧекпоинтовОркестратора(СсылкаДиалога, Лимит = 20) Экспорт
	Возврат ИИА_Сервер.ПолучитьИсториюЧекпоинтовОркестратора(СсылкаДиалога, Лимит);
КонецФункции

Функция ПолучитьDiffЧекпоинтовОркестратора(СсылкаДиалога, ИндексДо = -1, ИндексПосле = -1) Экспорт
	Возврат ИИА_Сервер.ПолучитьDiffЧекпоинтовОркестратора(СсылкаДиалога, ИндексДо, ИндексПосле);
КонецФункции

Функция ОркестраторВозобновитьСЧекпоинта(СсылкаДиалога, ИндексЧекпоинта = -1, АвтоЗапуск = Истина) Экспорт
	Возврат ИИА_Оркестратор.ВозобновитьСЧекпоинта(СсылкаДиалога, ИндексЧекпоинта, АвтоЗапуск);
КонецФункции

Функция ПолучитьМетрикиОркестратора(СсылкаДиалога) Экспорт
	Возврат ИИА_Сервер.ПолучитьМетрикиОркестратора(СсылкаДиалога);
КонецФункции

Функция ПолучитьЦепочкиСбоевОркестратора(СсылкаДиалога, Лимит = 5) Экспорт
	Возврат ИИА_Сервер.ПолучитьЦепочкиСбоевОркестратора(СсылкаДиалога, Лимит);
КонецФункции

Функция ПочемуОстановилсяОркестратор(СсылкаДиалога) Экспорт
	Возврат ИИА_Сервер.ПочемуОстановилсяОркестратор(СсылкаДиалога);
КонецФункции
